...
def online_platt_scaling_newton(preds, y, beta=0.1, D=1):
    ...
    for i in range(0, preds.size - 1):
        ...
        cum_hessian_inv = cum_hessian_inv - np.outer(cum_hessian_inv @ g_i, g_i.T @ cum_hessian_inv) / (
...
def online_beta_scaling_newton(preds_1, preds_2, y, beta=0.1, D=2):
    ...
    for i in range(0, preds_1.size - 1):
        ...
        pred_probs_platt[i + 1] = sigmoid(
            online_platt_a[i + 1] * preds_1[i + 1] + online_platt_b[i + 1] * preds_2[i + 1] + online_platt_c[i + 1])
